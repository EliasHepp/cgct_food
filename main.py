...
def BAE(data, params):
    ...
    # Model - Reverse Gradients To Assure Representation Is Non-Predicitve of Treatment
    ...
    #Encoding, Decoding, Treatment Predictor
    ...
    #add huber loss to check for outliers
    huber_loss = nn.HuberLoss(delta=1.0)  
    
    # Train the model
    for epoch in range(n_epochs):
        total_loss, total_lossT, total_lossX, total_lossHuber = 0, 0, 0, 0  
        for batch in train_loader:
            ...
            lossHuberX = huber_loss(x_out, x_data) 
            lossHuberT = huber_loss(t_out, t_data)
            loss = lossX + alpha*lossT
            lossHuber = lossHuberX + alpha*lossHuberT
            
            total_loss += loss.item()
            total_lossX += lossX.item()
            total_lossT += lossT.item()
            total_lossHuber += lossHuber.item()  # Accumulate Huber loss
...
def SCw(data, aid_random, params):
    ...
    for i in range(n):
        for j in range(aid_dim):
            ...
            w_opt = scipy.optimize.minimize(fun=optim_fn, x0=(1/n)*np.ones((n,1)).flatten(),#.reshape((n,1)), 
...
def REGFOR(data, params):
    regfor = RegressionForest(n_estimators=params['n_estimators'],
    ...
    regfor_model = regfor.fit(X, Y)
    regfor_imp = regfor_model.feature_importances()
    regfor_path = regfor_model.decision_path(X)
    
    return regfor_model, regfor_imp, regfor_path
...
def REGFOR_pred(newdata, model):
    
    regfor_model = model
    Y_pred = regfor_model.predict(newdata)
...
def CAUSFOR(newdata, params):
    # estimated effects are marginal?
    ...
    cf_model = CausalForestDML(model_t=RandomForestRegressor(),
    ...
    return cf_model, treatment_effects, importances 