    ...
    if model == 'rf':  
        for n_estimators in hyperpar_list['n_estimators']:
            for min_samples_leaf in hyperpar_list['min_samples_leaf']:
                ...
                Y_pred = np.empty((data_val_a.shape[0], data_val_a.shape[1]))
                for i in range(data_val_a.shape[1]):
                    Y_pred[:,i] = REGFOR_pred(np.concatenate([data_val_a[:,i:(i+1)], data_val_x], axis=1), model).ravel() 
                Y_true = data_val_y
                loss = RMISE(Y_pred, Y_true)
                                            ...
                                            for mscw in hyperpar_list['m_scw']:
                                                ...
                                                for n_estimators in hyperpar_list['n_estimators']:
                                                    for min_samples_leaf in hyperpar_list['min_samples_leaf']:
                                                        ...
                                                        Y_pred = np.empty((data_val_a.shape[0], data_val_a.shape[1]))
                                                        for i in range(data_val_a.shape[1]):
                                                            Y_pred[:,i] = REGFOR_pred(np.concatenate([data_val_a[:,i:(i+1)], data_val_bae_x], axis=1), model).ravel() 
                                                        Y_true = data_val_y
                                                        loss = RMISE(Y_pred, Y_true)
                                ...
                                for albae in hyperpar_list['alpha_bae']:
                                    ...
                                    for n_estimators in hyperpar_list['n_estimators']:
                                        for min_samples_leaf in hyperpar_list['min_samples_leaf']:
                                            ...
                                            model, _, _ = REGFOR(data_train_bae, hyperpars)
